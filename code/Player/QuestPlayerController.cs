...
public partial class QuestPlayerControllerSimple : BasePlayerController
{
	...
	public static float BodyGirth { get; set; } = 32f;
	public static float BodyHeight { get; set; } = 72f;
...
public override void Simulate()
	{
		...
		{
			Log.Info( "uh oh" );
			Position = Position.WithZ( z -= GroundDistance );
		}
		else
		{
...
/*DebugOverlay.ScreenText( $"[QUEST WALK CONTROLLER]\n" +
		$"Velocity:                    {Velocity}\n" +
...
{
// Get a Vector3 point directly below the player's position.
Vector3 point = new Vector3( Position ).WithZ( Position.z - GroundDistance );

// Trace a BBox from the player's position to the ground below it.
TraceResult trace = TraceBBox( Position, point );


// If the trace didn't hit anything, we are in free fall.
// Set the GroundEntity to null.
...
{
// Set the GroundEntity to the passed in entity.
GroundEntity = entity;
}

private void CategorizePosition()
{
var point = Position + Vector3.Down * 1f;
...
// Set the end position to the ground.
